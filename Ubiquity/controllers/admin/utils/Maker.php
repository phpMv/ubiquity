	...
	public static function createAction($controller, $action, $parameters = "", $content = "", $createView = false, $route = []) {
		...
		if ($posLast !== false) {
			...
			if (isset ( $route ["path"] )) {
				...
				$routeAnnotation = UFileSystem::openReplaceInTemplateFile ( self::$baseDir . "/admin/templates/annotation.tpl", [ "%name%" => $name,"%properties%" => $routeProperties ] );
			}
			$parameters = CodeUtils::cleanParameters ( $parameters );
			$actionContent = UFileSystem::openReplaceInTemplateFile ( self::$baseDir . "/admin/templates/action.tpl", [ "%route%" => "\n" . $routeAnnotation,"%actionName%" => $action,"%parameters%" => $parameters,"%content%" => $content ] );
			$fileContent = \substr_replace ( $fileContent, "\n%content%", $posLast - 1, 0 );
			if (! CodeUtils::isValidCode ( '<?php ' . $content )) {
				$msgContent = [ "Errors parsing action content!" ];
				$hasErrors = true;
			} else {
