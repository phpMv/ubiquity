...
trait SeoTrait{
	...
	public function displaySiteMap(...$params){
		...
		Session::set("seo-sitemap", $controllerSeo);
		...
		$dt->setCheckedCallback(function($object){return $object->getExisting();});
		$dt->asForm();
	...
	public function generateRobots(){
		...
		if (Request::isPost()) {
			...
			$urls=Request::post('selection',[]);
			foreach ($urls as $url){
				$content[]=\str_replace("%url%", Request::cleanUrl($siteUrl.$url), $template);
			}
			if(\sizeof($content)>0){
	...
	public function createSeoController($force=null) {
		if (Request::isPost()) {
			...
			$variables["%urlsFile%"]=Request::post("urlsFile","urls");
			$variables["%sitemapTemplate%"]=Request::post("sitemapTemplate","Seo/sitemap.xml.html");
	...
	public function saveUrls(){
		$result=[];
		$selections=Request::post("selection",[]);
		$locations=Request::post("location",[]);
		$lastModified=Request::post("lastModified",[]);
		$changeFrequency=Request::post("changeFrequency",[]);
		...
		$seoController=Session::get("seo-sitemap");
